...
```yaml
# Необязательный блок переменных, доступных как `{{ global('имя') }}` и `{{ local('имя') }}`.
# - `global` — выражение вычисляется один раз (при первом обращении) и переиспользуется для всех документов.
# - `local` — выражение вычисляется заново для каждого генерируемого документа.
variables:
  global:
    shared_id: "{{ uuid }}"
    shared_suffix: "{{ global('shared_id') }}-{{ number(1, 10) }}"
  local:
    base_prefix: "{{ source_extracted }}"
...
В значениях секций `value_override` можно указывать как строковое значение, так и специальные плейсхолдеры:

- `{{ function }}` — подставит значение, предоставленное предопределенной функцией function.
- `{{ global('имя') }}` — вернёт значение глобальной переменной `имя`, объявленной в блоке `variables.global`. Выражение
  этой переменной вычисляется один раз (при первом обращении) и далее переиспользуется.
- `{{ local('имя') }}` — вернёт значение локальной переменной `имя`, объявленной в блоке `variables.local`. Выражение
  пересчитывается перед генерацией каждого документа. Через `local()` также доступны встроенные значения контекста:
  `root_element`, `source_filename`, `source_extracted`, `output_filename`.

Блок `variables`, располагающийся в корне YAML-файла, заменяет устаревший синтаксис с `| global`/`| local` и позволяет явным образом задавать значения,
которые потом можно переиспользовать в шаблонах.
...
| `local('имя')`                     | Значение локальной переменной `имя` (из `variables.local` или встроенных значений контекста)               |
