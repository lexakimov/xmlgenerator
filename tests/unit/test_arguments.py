...
def test_parse_args_02(capsys):
    args, xsd_files, output_path = parse('program ../existing.xsd')
...
def test_parse_args_02_1(capsys):
    args, xsd_files, output_path = parse('program ../existing.xsd ../existing_2.xsd')
...
def test_parse_args_02_2(capsys):
    args, xsd_files, output_path = parse('program ../existing.xsd ../existing.xsd')
...
def test_parse_args_03(capsys):
    args, xsd_files, output_path = parse('program ../')
    ...
    assert args.seed is None
    assert len(xsd_files) is 2
    assert [v.name for v in xsd_files] == ['existing.xsd', 'existing_2.xsd']
    assert output_path is None
    captured = capsys.readouterr()
    assert not captured.out
    assert not captured.err
...
def test_parse_args_05(capsys):
    args, xsd_files, output_path = parse('program -o out.xml ../existing.xsd')
...
def test_parse_args_06(capsys):
    with pytest.raises(SystemExit) as excinfo:
        parse('program -o out.xml ../existing.xsd ../existing_2.xsd')
...
def test_parse_args_07(capsys):
    ...
    parse('program -o output_dir/ ../existing.xsd ../existing_2.xsd')
    assert Path("output_dir").exists()
    Path("output_dir").rmdir()


def test_parse_args_08(capsys):
    args, xsd_files, output_path = parse('program -c ../config.yaml ../existing.xsd')
...
def test_parse_args_09(capsys):
    with pytest.raises(SystemExit) as excinfo:
        parse('program -c not_exists.yml ../existing.xsd')
